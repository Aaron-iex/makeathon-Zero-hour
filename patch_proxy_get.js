...
const regex = /\/\/ 1\. Read from Supabase FIRST[\s\S]*?\/\/ 2\. Fallback to Sheets/m;

const replacement = `// FALLBACK: Temporarily bypass Supabase and read directly from Sheets
    ...
    // Ignore old fallback to Sheets`;

if (regex.test(code)) {
  code = code.replace(regex, replacement);
  // Also remove the old fetch code below "// Ignore old fallback to Sheets"
  const oldFetchRegex = /\/\/ Ignore old fallback to Sheets[\s\S]*?\}\s*\n\s*\/\/ ── POST/m;
  if (oldFetchRegex.test(code)) {
    code = code.replace(oldFetchRegex, "// ── POST");
  }
