...
import { REGISTRATION_CLOSED } from "./RegisterDialog";

const TICKER_ITEMS = [
  REGISTRATION_CLOSED
    ? "EVENT DATE: SEPT 23 // SQUAD ENROLLMENT TERMINATED — FULL CAPACITY"
...
export function EmergencyTicker() {
  ...
  return (
    <aside className="relative flex h-8 w-full items-center overflow-hidden border-b border-primary/40 bg-card/90 font-mono-tech text-[11px] tracking-[0.18em] text-primary select-none">
      <div className="z-10 flex h-full shrink-0 items-center gap-2 bg-primary px-3 sm:px-3.5 text-primary-foreground shadow-[4px_0_16px_rgba(224,76,17,0.3)]">
        ...
        <span className="hidden sm:inline-block font-mono-tech text-[8px] border border-black/30 bg-card/80 px-1.5 py-0.5 text-white tracking-widest">
          TX {txHex}
        </span>
      </div>
